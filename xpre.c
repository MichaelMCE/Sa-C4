...
void dumpHex16 (const unsigned char *data, size_t length)
{
...
int xpre_prepare (xpre_t *xpre, const char *buffer, const int bufferLen)
{
	...
	xpre->buffer = calloc(1, xpre->bufferLen);
...
int xpre_prepareBuffer (xpre_t *xpre, void *buffer, const int bufferLen)
{
...
{
	size_t flength = 0;
	char *fileXML = xpre_loadFile(path, &flength);
	if (!fileXML){
...
int32_t xpre_getInt32 (xpre_t *xpre, const char *field)
{
	...
	int32_t value = 0;
	int ret = sscanf(location, "%i<", &value);
	if (ret == EOF)
		printf("<%s> not found\n", field);
...
{
	size_t flength = 0;
	char *fileXML = loadFile("data/future.pre", &flength);
	if (!fileXML){
	...
	xpre_t xpre;
	xpre_init(&xpre);
	xpre_prepare(&xpre, fileXML, flength);
	int len = xpre_preset2bin(&xpre, buffer, XPRE_FORMAT_NAME/*|XPRE_FORMAT_WRITE*/, 0);
	
	dumpHex(buffer, len);

	as_preset_t *preset = (as_preset_t*)buffer;
	printf("name : '%s'\n", preset->name);
	//printf("name : '%s'\n", &buffer[160]);		// if formatwrite
	printf("final length %i %i\n", len, (int)sizeof(as_preset_t));
	as_preset_t pre = *preset;
	xpre_cleanup(&xpre);
	

	uint8_t zbuffer[512*1024];
	xpre_init(&xpre);
	xpre_prepareBuffer(&xpre, zbuffer, sizeof(zbuffer));
	xpre_bin2string(&xpre, &pre, sizeof(pre));
	//printf("'%s'\n", pre.name);	
	//printf("%s\n", xpre.buffer);
